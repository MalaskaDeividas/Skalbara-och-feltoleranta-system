...
export async function VerifyUser (username: string, password:string): Promise<boolean | string>
{
  try 
  {
    const respone = await axios.post('http://9.223.187.114/api/user/login', {
      username: username,
      password: password
    });
...
export async function CreateUser (name:string, lastname:string, username:string, age: number, password:string, isAdmin:boolean) : Promise<boolean>
{
  try 
  {
    const respone = await axios.post('http://9.223.187.114/api/user/signup',{
      username: username,
      password: password,
      name: name,
      lastname:lastname,
...
export async function DeleteUser(username: string) {
  try {
    ...
    const response = await axios.delete('http://9.223.187.114/api/user/deleteme', {
